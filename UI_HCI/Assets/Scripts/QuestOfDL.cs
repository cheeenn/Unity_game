...
public class QuestOfDL : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        IsQuestTurnIn = false;
        DisplaySign = true;
        QuestName = "* Find the Ball of Yarn";
        QuestDescription = "Dandy Lion lost his favorite ball of yarn. Please help him find it";
        QuestRequirement = "Yarn";
        QuestReq = 1;
        QuestCounter = 0;
        IsDiaOver = false;
        // Find GameObject
        ...
        if (near)
        {
            ...
            if (!IsQuestAccept && !IsQuestComplete)
            {
                ...
                if (Input.GetKeyDown(KeyCode.C) && !Dialog)
                {
                    ...
                    Dia_Text.text = "What a purrfect day it is!";
                    state = 1;
                }
                switch (state)
                {
                    case 1:
                        ...
                        {
                            Text Dia_Text = Dialog.transform.GetChild(4).GetComponent<Text>();
                            Dia_Text.text = "Wait …  I can’t beleaf it.  I lost my favorite ball of yarn.  Can you help me find it? ";
                            state = 2;
                        }
                        break;
                    case 2:
                        if (Input.GetKeyDown(KeyCode.Return))
                        {
                            IsQuestAccept = true;
                            // Add quest to player
                            chr.SendMessage("AddQuest", "DandyLion");
                            // 
                            DestroyCanvas();
                            UnFreezeNim();
                            //DisplaySign = true;
            ...
            else if (IsQuestAccept && !IsQuestComplete)
            {
                ...
                if (Input.GetKeyDown(KeyCode.C))
                {
                    ...
                    GenerateDialog();
                    Text Dia_Text = Dialog.transform.GetChild(4).GetComponent<Text>();
                    Dia_Text.text = "My yarn should be around here somewhere...";
                    state = 1;
                }
                switch (state)
                {
            ...
            else
            {
                ...
                if (Input.GetKeyDown(KeyCode.C))
                {
                    ...
                    GenerateDialog();
                    Text Dia_Text = Dialog.transform.GetChild(4).GetComponent<Text>();
                    Dia_Text.text = "Thanks for helping meowt!";
                ...
                switch (state)
                {
                    case 1:
                        if (Input.GetKeyDown(KeyCode.Return))
                        {
                            ...
                            Name.text = "";
                            Dia_Text.text = "Dandy Lion's wish is granted!";
    ...
    void GenerateDialog()
    {
        if (!Dialog)
        {
            ...
            Right.sprite = DandyLion;
            Text Name = Dialog.transform.GetChild(3).GetComponent<Text>();
            Name.text = "Dandy Lion";
        }
    }
